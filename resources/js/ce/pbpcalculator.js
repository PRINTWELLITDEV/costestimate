...
$(function () {
    ...
    function populateStocksBySite(siteCode) {
        ...
        let html = '<option value="" selected disabled>Select Stock</option>';
        filtered.forEach((s) => {
            html += `
    ...
    const isLevel1 = Number($("#pbpCalculatorForm").data("user-level")) === 1;
    ...
    function recalculateRow($tr) {
        ...
        const otherChargesPct = Number($('input[name="OtherChargesRate"]').val() || 0);
        const sheetingInput = Number($('input[name="SheetingCost"]').val() || 0);
        ...
        const excludeDuty = $tr.find(".js-excludeduty-toggle").is(":checked");
        const applySheeting = um === "RL" || (um === "SH" && $tr.find(".js-sheeting-toggle").is(":checked"));
        ...
        const CostPerSheet = SheetsPerMT > 0 ? SheetedCost / SheetsPerMT : 0;
    ...
    function renderCalculatorRows(rows) {
        ...
        if (!rows || rows.length === 0) {
            $tbody.html('<tr><td colspan="21" class="text-muted">No matching pricing found.</td></tr>');
            return;
        }
    ...
    $(document).on("click", "#addToEstimateBtn", function () {
        ...
        const raw = Number($row.find(".js-cost-per-sheet").data("value") || 0);
    ...
    $(document).on("submit", "#pbpCalculatorForm", function (e) {
        ...
        $.ajax({
            ...
            error: function (xhr) {
                const msg = xhr.responseJSON?.message || "Calculation failed.";
