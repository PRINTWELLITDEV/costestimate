@forelse($pricings as $pricing)
    <tr data-id="{{ $pricing->id }}" data-site="{{ $pricing->Site }}" data-vendorgroup="{{ $pricing->VendorGroup }}"
        data-ptype="{{ $pricing->PType }}" data-vendor="{{ $pricing->Vendor }}" data-itemcode="{{ $pricing->ItemCode }}"
        ...
        <td>
            <div class="d-flex gap-2">
                <div class="align-items-center">
                    <div class="fw-bold">
                        {{ $pricing->ItemCode ?? 'N/A' }}
                    </div>
                    <div class="small">
                        {{ $pricing->ItemDesc ?? 'N/A' }}
                    </div>
                </div>
            </div>
        </td>
        ...
        <td>{{ number_format($pricing->Price_MT, 2) }}</td>
        <td>{{ number_format($pricing->Price_Sheet, 2) }}</td>
        <td>{{ number_format($pricing->Price_Pound, 2) }}</td>
        <td>{{ number_format($pricing->Price_Bale, 2) }}</td>
                ...
                <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-end" aria-labelledby="">
                    ...
                    <li>
                        <a href="#" class="dropdown-item delete-pricing-btn" data-site="{{ $pricing->Site }}" data-id="{{ $pricing->id }}"
                            ...
                            data-itemcode="{{ $pricing->ItemCode }}">
